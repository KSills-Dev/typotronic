...
\Fn{\findtypos{\correct, \actual}}{
  Let \data be a new integer array with dimensions (\len{\correct}, \len{\actual})\\
  \For{i \KwFrom \len{\correct} \downto 1}{
    \For{j \KwFrom \len{\actual} \downto 1}{
      \uIf{$i=1$ and $j\ne1$} {
      ...
      \uElseIf{$i \ne 1$ and $j=1$}{
        Let the cost be $\deletecost(i,j) + ddata[i+1, j]$\\
        Store the cost in $data[i,j]$\\
        Set the typo of $data[i,j]$ to \KwDelete\\
        Set the parent of $data[i,j]$ to $(i+1, j)$\\
      }
      ...
      \uIf{(i, j) is (1,1)}{
        \return the cost in $data[1,1]$ chain of parents starting in that cell\\
      }
    }
  }
}
