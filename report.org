...
#+BEGIN_EXPORT latex
  ...
  \left\{
  \begin{aligned}
  & C(pattern[1..m-1], typo) + insertCost(pattern[m], pattern[m-1]), \\
  & C(pattern, typo[1..n-1]) + deleteCost(typo[n], typo[n-1]), \\
  & C(pattern[1..m-1], typo[1..n-1]) + substituteCost(pattern[m], typo[n]), \\
  & C(pattern[1..m-1], typo[1..n-2]\, ||\, typo[n]) + transposeCost(typo[n-1], typo[n])
  \end{aligned}
  \right\}, & m \ge 1, n \ge 1 \\

  \left\{ C(pattern[1..m-1], typo) + insertCost(pattern[m], pattern[m-1]) \right\}, & m = 0, n \ge 1 \\
  \left\{ C(pattern, typo[1..n-1]) + deleteCost(typo[n], typo[n-1]) \right\}, & m \ge 1, n = 0 \\
  {0}, & m = 0, n = 0
  \end{cases}
  \]
#+END_EXPORT

(Note that $||$ is the string concatenation operator here)
...
We ended up slightly modifying the transposition recurrence:
